...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        ...
        filedirs = [f.split('_')[0] for f in os.listdir(data_dir) if f.endswith('clean.npy') ]
        # print('idcs',idcs, len(idcs))
        self.filenames = [os.path.join(data_dir, '%s_clean.npy' % idx) for idx in filedirs]
        print('data_dir',data_dir)
        labels = []
        
        for idx in filedirs:
            l = np.load(os.path.join(data_dir, '%s_label.npy' %idx))#空的
